...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockCatalogRepository.getProducts(categoryId: any(named: 'categoryId')))
        .thenAnswer((_) async => const Success([]));
    when(() => mockCatalogRepository.getProducts())
        .thenAnswer((_) async => const Success([]));
    when(() => mockCartRepository.getCartItems(any()))
        .thenAnswer((_) async => const Success([]));
    when(() => mockOrderRepository.watchUserOrders(any()))
